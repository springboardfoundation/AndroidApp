...
import com.onegravity.contactpicker.util.DelimiterUtil;
...
import static com.onegravity.contactpicker.util.Util.getSimCountryIso;
...
public class Address implements Parcelable, Comparable<Address> {
  ...
  ExternalAddressFormatter getExternalAddressFormatter(Context context) {
    String localNumber = SharedPrefsUtils.getStringPreference(context, ActivityConstants.PREF_MOBILE_NUMBER);
    ...
    if (!TextUtils.isEmpty(localNumber)) {
      ...
      return new ExternalAddressFormatter(getSimCountryIso(context), true);
    }
  }
  ...
  public static class ExternalAddressFormatter {
    ...
    ExternalAddressFormatter(@NonNull String localNumberString) {
      try {
        ...
        if(!localNumberString.contains("+91")) {
          Log.i("shoeb", localNumberString);
        }
